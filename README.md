...
- **Features**:
  - **Memory**: Displays free GB with color coding based on system pressure. The dropdown also names the top memory-consuming process (`Top Memory: Messages (507M)`), click-to-kill like the CPU list; `kernel_task`, `WindowServer`, and Chrome are excluded as never-interesting/never-useful-to-kill.
  ...
  - **Top Processes**: Lists the top CPU consumers; automatically surfaces the top non-kernel process in the menu bar if CPU usage exceeds 50%. Click any process in the dropdown to kill it — a confirmation dialog appears, then SIGKILL (`-9`) is sent, so even a process that traps or ignores SIGTERM dies immediately; the plugin then redraws right away instead of waiting out the tick.
  ...
  - **SwiftBar death ledger**: the watchdog above catches a host that is *too busy*; this catches one that is simply **gone**. SwiftBar crashes do leave an `.ips` in `~/Library/Logs/DiagnosticReports/`, but macOS moves them into `Retired/` within the hour and purges that soon after — so each death erases the evidence of the last one, and "it keeps dying" becomes unanswerable. Every tick this **harvests any new report into a durable log before it vanishes**, parsing the crash time, exception/signal, the human-readable reason from `asi` (`SIGTRAP: BUG IN CLIENT OF LIBMALLOC: memory corruption of free block`), how long the host had been up, and the app version — from the first 50 lines only, so the ~180 lines of thread dumps are never read. Deaths that leave *no* report at all (a clean quit, a hang killed by hand, the spin watchdog, a logout) are caught separately by noticing a changed host PID between ticks, and are labelled `watchdog` when `host_restarts` explains them; a crash already logged in the same window is never double-counted. Once there is anything to answer for, the dropdown leads with `✕ SwiftBar died 3× in 7d — last 41m ago` over a submenu of the last 8, each with cause, uptime, and version; it is silent while the host behaves. Unlike every other stateful part of this plugin, the log lives outside `$TMPDIR` — in `~/Library/Application Support/xbar-metrics/host_deaths`, 90 days of it — because `$TMPDIR` is wiped at every reboot and a reboot is exactly when you most want the history. Necessarily retroactive: the recorder is itself a SwiftBar plugin, so a death is logged on the first tick after the host is back, not while it is down.
